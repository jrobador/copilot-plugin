...
// A successful look at the tree tells the watcher nothing it would act on.
// A failing one still shows.
const QUIET_READ = /^Ran command: (git (status|diff|log|show)|ls)\b.*\(exit 0\)$/;
 ...
 */
export function classifyLogLine(line, { allCommands = false } = {}) {
  const match = LOG_PREFIX.exec(String(line).trimEnd());
  if (!match) {
    return null;
  }
  const text = match[1].replace(/\.$/, "");
  if (!allCommands && QUIET_READ.test(text)) {
    return null;
  }
