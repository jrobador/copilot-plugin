...
```bash
node "${CLAUDE_PLUGIN_ROOT}/bin/copilot-plugin.mjs" task-resume-candidate --json
```
...
- The subagent is a thin forwarder only. It should use one `Bash` call to invoke `node "${CLAUDE_PLUGIN_ROOT}/bin/copilot-plugin.mjs" task ...` and return that command's stdout as-is.
- Return the Copilot plugin stdout verbatim to the user.
