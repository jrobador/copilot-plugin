...
const REVIEW_PROPS = {
  ...PATH_PROP,
  // Safe to expose to a model: it selects what to review, never widens the
  // fence, and a wrong number fails closed -- the review refuses unless the
  // pull request's head commit is already checked out, which only a person
  // can arrange. `--add-dir` is deliberately absent for the opposite reason.
  pr: {
    type: "integer",
    minimum: 1,
    description:
      "Review this pull request number. Requires the GitHub CLI (gh) and that the pull request's head is already checked out (`gh pr checkout <n>`); otherwise the review refuses rather than judging the wrong commit."
  },
  base: { type: "string", description: "Review the branch diff against this ref instead of the working tree. With pr, overrides the pull request's base branch." },
  scope: { type: "string", enum: ["auto", "working-tree", "branch"], description: "What to review. Default: auto. Cannot be combined with pr." },
  ...MODEL_PROPS
};
...
export const TOOLS = [
  {
    ...
    toArgv: (a) => ["review", ...opt("--pr", a.pr), ...opt("--base", a.base), ...opt("--scope", a.scope), ...opt("--model", a.model), ...opt("--effort", a.effort)]
  },
  {
