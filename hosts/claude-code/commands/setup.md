...
```bash
node "${CLAUDE_PLUGIN_ROOT}/bin/copilot-plugin.mjs" setup --json $ARGUMENTS
```
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/bin/copilot-plugin.mjs" setup --install-runtime --json $ARGUMENTS
```
