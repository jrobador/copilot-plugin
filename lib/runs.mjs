/**
 * The orchestration behind the plugin's subcommands: a review run, a task
 * run, and the resume of a task that paused on the owner's approval.
 *
 * Lifted out of copilot-plugin.mjs unchanged so the flows can run in tests
 * against the fake SDK (see COPILOT_PLUGIN_SDK_MODULE). The plugin keeps
 * the argv parsing, job bookkeeping and output; this module keeps the part
 * that talks to Copilot.
 */

import fs from "node:fs";
import path from "node:path";

import {
  buildPersistentTaskSessionId,
  buildTaskPayload,
  DEFAULT_CONTINUE_PROMPT,
  createSession,
  runPrompt,
  getCopilotLoginStatus,
  getSdkStatus,
  listModels,
  parseStructuredOutput,
  READ_ONLY,
  resumeSession,
  WORKSPACE_EXECUTE,
  WORKSPACE_WRITE
} from "./copilot-client.mjs";
import { collectReviewContext, ensureGitRepository, resolveReviewTarget } from "./git.mjs";
import { sortJobsNewestFirst } from "./job-control.mjs";
import {
  createWorkspacePolicy,
  findOutsidePathsInText,
  isWideRoot,
  resolveAdditionalDirectories
} from "./paths.mjs";
import { makeSentinelEscalation } from "./permissions.mjs";
import { allowedPrograms, missingPrograms } from "./run-command.mjs";
import { REVIEW_SCHEMA, ROOT_DIR } from "./plugin-root.mjs";
import { loadPromptTemplate, interpolateTemplate } from "./prompts.mjs";
import { renderReviewResult, renderTaskResult } from "./render.mjs";
import { COMPLETED_DEGRADED, getConfig, listJobs } from "./state.mjs";
import { resolveWorkspaceRoot } from "./workspace.mjs";

// Matches the SDK's ReasoningEffort type exactly. The previous set
// (none, minimal, ...) was inherited from Codex: "minimal" passed our
// validation and then failed inside the SDK, while "max" -- which the SDK
// accepts -- was rejected here.
export const VALID_REASONING_EFFORTS = new Set(["low", "medium", "high", "xhigh", "max"]);
// Convenience aliases. Real ids come from `client.listModels()`; these only
// spare the user from typing an exact version string.
export const MODEL_ALIASES = new Map([
  ["codex", "gpt-5.3-codex"],
  ["gemini", "gemini-3.1-pro-preview"],
  ["sonnet", "claude-sonnet-5"],
  ["opus", "claude-opus-5"]
]);
export const STOP_REVIEW_TASK_MARKER = "Run a stop-gate review of the previous Claude turn.";

export function normalizeRequestedModel(model) {
  if (model == null) {
    return null;
  }
  const normalized = String(model).trim();
  if (!normalized) {
    return null;
  }
  return MODEL_ALIASES.get(normalized.toLowerCase()) ?? normalized;
}

export function normalizeReasoningEffort(effort) {
  if (effort == null) {
    return null;
  }
  const normalized = String(effort).trim().toLowerCase();
  if (!normalized) {
    return null;
  }
  if (!VALID_REASONING_EFFORTS.has(normalized)) {
    throw new Error(
      `Unsupported reasoning effort "${effort}". Use one of: low, medium, high, xhigh, max.`
    );
  }
  return normalized;
}

function shorten(text, limit = 96) {
  const normalized = String(text ?? "").trim().replace(/\s+/g, " ");
  if (!normalized) {
    return "";
  }
  if (normalized.length <= limit) {
    return normalized;
  }
  return `${normalized.slice(0, limit - 3)}...`;
}

function firstMeaningfulLine(text, fallback) {
  const line = String(text ?? "")
    .split(/\r?\n/)
    .map((value) => value.trim())
    .find(Boolean);
  return line ?? fallback;
}

/**
 * Both review prompts declare a JSON output contract. The schema file is the
 * single source of that contract, so inject it rather than describing it twice
 * (upstream referred to "the provided schema" but never provided it).
 */
export function buildReviewPrompt(templateName, { reviewName, context, focusText }) {
  const template = loadPromptTemplate(ROOT_DIR, templateName);
  return interpolateTemplate(template, {
    REVIEW_KIND: reviewName,
    TARGET_LABEL: context.target.label,
    USER_FOCUS: focusText || "No extra focus provided.",
    OUTPUT_SCHEMA: fs.readFileSync(REVIEW_SCHEMA, "utf8").trim(),
    REVIEW_INPUT: context.content
  });
}

/** error.code values the CLI's top-level handler acts on. */
export const COPILOT_RUNTIME_MISSING = "COPILOT_RUNTIME_MISSING";
export const COPILOT_AUTH = "COPILOT_AUTH";

export async function ensureCopilotReady(cwd) {
  const authStatus = await getCopilotLoginStatus(cwd);
  if (!authStatus.available) {
    throw Object.assign(new Error(`The Copilot runtime is unavailable: ${authStatus.detail}`), { code: COPILOT_RUNTIME_MISSING });
  }
  if (!authStatus.loggedIn) {
    throw Object.assign(
      new Error(`Copilot is not authenticated (${authStatus.detail}). Run \`!copilot login\`, then rerun \`/copilot:setup\`.`),
      { code: COPILOT_AUTH }
    );
  }
  return authStatus;
}

/**
 * Fail early on a model this account cannot use.
 *
 * Without this the session starts, the turn runs, and the failure surfaces from
 * deep inside the SDK with no hint about which ids are valid. The account's own
 * model list is the answer, so show it.
 */
export async function ensureModelAvailable(cwd, model) {
  if (!model) return;

  const models = await listModels(cwd);
  if (models.length === 0) return; // Could not enumerate; let the run decide.

  const ids = models.map((entry) => entry.id ?? entry.name).filter(Boolean);
  if (ids.includes(model)) return;

  throw new Error(
    [
      `Model "${model}" is not available on this account.`,
      `Available models: ${ids.join(", ")}.`,
      `Aliases: ${[...MODEL_ALIASES.keys()].join(", ")}.`
    ].join("\n")
  );
}

/**
 * The Copilot session id of a job, whichever name it was stored under.
 * `sessionId` is deliberately not consulted: that is the Claude Code session.
 */
export function copilotSessionIdOf(job) {
  return job?.copilotSessionId ?? job?.threadId ?? null;
}

/** Is this a finished task whose Copilot session a follow-up can pick up? */
export function isResumableTask(job) {
  if (job?.jobClass !== "task" || !copilotSessionIdOf(job)) return false;
  // A degraded task is finished and has a live thread: continuing it (after an
  // --add-dir, say) is exactly what the owner will want to do next.
  return job.status === "completed" || job.status === COMPLETED_DEGRADED;
}

export async function resolveLatestTrackedTaskSession(workspaceRoot, options = {}) {
  const jobs = sortJobsNewestFirst(listJobs(workspaceRoot)).filter((job) => job.id !== options.excludeJobId);
  const activeTask = jobs.find((job) => job.jobClass === "task" && (job.status === "queued" || job.status === "running"));
  if (activeTask) {
    throw new Error(`Task ${activeTask.id} is still running. Use /copilot:status before continuing it.`);
  }

  const trackedTask = jobs.find(isResumableTask);
  if (trackedTask) {
    return { id: copilotSessionIdOf(trackedTask), jobId: trackedTask.id };
  }

  return null;
}

/**
 * Refuse before the turn, not during it.
 *
 * A prompt that names a file outside the fence is a wall the model would
 * otherwise find by walking into it -- that is the shape of the run that spent
 * 481k input tokens arguing with a denial. `--add-dir` is the remedy, so the
 * error names it with the directory already filled in.
 */
export function assertPromptPathsInsideFence(text, policy) {
  const outside = findOutsidePathsInText(text, policy);
  if (outside.length === 0) return;
  const named = outside.map((entry) => entry.token).join(", ");
  const dirs = [...new Set(outside.map((entry) => path.dirname(entry.resolved)))];
  throw new Error(
    `The prompt names paths outside this job's fence: ${named}. Every read there would be refused. Re-run with ${dirs
      .map((dir) => `--add-dir ${dir}`)
      .join(" ")}, or start the job from a directory that contains them.`
  );
}

/**
 * Warn about allowed programs this machine does not have. Only an empty
 * intersection is fatal: that is a broken PATH, not a missing toolchain.
 */
export function reportMissingPrograms(mode, extraPrograms, onProgress) {
  const config = { extraPrograms };
  const missing = missingPrograms(mode, config);
  if (missing.length === 0) return missing;
  if (missing.length === allowedPrograms(mode, config).length) {
    throw new Error(
      `None of this job's allowed programs resolve on PATH (${missing.join(", ")}). The job could not run a single command; check the PATH of the process that started it.`
    );
  }
  onProgress?.({
    message: `Not on PATH, so unavailable to this job: ${missing.join(", ")}.`,
    phase: "starting",
    stderrMessage: `Missing programs: ${missing.join(", ")}`,
    logTitle: null,
    logBody: null
  });
  return missing;
}

/**
 * Everything a run depends on, checked without creating a session, spending a
 * turn or writing a job record. Collects every problem instead of throwing on
 * the first, so one invocation shows the whole list.
 */
export async function dryRunReport(request = {}) {
  const checks = [];
  const add = (level, label, detail) => checks.push({ level, label, detail });
  const cwd = request.cwd ?? process.cwd();
  const workspaceRoot = request.workspaceRoot ?? resolveWorkspaceRoot(cwd);

  if (!isWideRoot(workspaceRoot)) {
    add("ok", "Workspace root", workspaceRoot);
  } else if (request.write && !request.allowWideRoot) {
    add(
      "error",
      "Workspace root",
      `${workspaceRoot} is your home directory, an ancestor of it, or a drive root, so a --write job is refused there. Run from a project directory, or pass --allow-wide-root.`
    );
  } else {
    add("warn", "Workspace root", `${workspaceRoot} is wide: "inside the workspace" covers nearly everything you own.`);
  }

  const resolvedDirs = [];
  for (const dir of Array.isArray(request.addDirs) ? request.addDirs : []) {
    try {
      const [resolved] = resolveAdditionalDirectories([dir], cwd);
      resolvedDirs.push(resolved);
      add("ok", "--add-dir", resolved);
    } catch (error) {
      add("error", "--add-dir", error.message);
    }
  }

  const policy = createWorkspacePolicy(workspaceRoot, cwd, resolvedDirs);
  add("ok", "Fence", policy.roots.map((entry) => entry.resolved).join(", "));

  try {
    assertPromptPathsInsideFence(request.prompt, policy);
    add("ok", "Paths named in the prompt", "all inside the fence");
  } catch (error) {
    add("error", "Paths named in the prompt", error.message);
  }

  const mode = request.permissionMode ?? (request.write ? WORKSPACE_WRITE : READ_ONLY);
  const extraPrograms = getConfig(workspaceRoot).extraPrograms ?? [];
  const allowed = allowedPrograms(mode, { extraPrograms });
  add("ok", "Permission mode", `${mode} (programs: ${allowed.join(", ")})`);

  const missing = missingPrograms(mode, { extraPrograms });
  if (missing.length === 0) {
    add("ok", "Programs on PATH", "all present");
  } else if (missing.length === allowed.length) {
    add("error", "Programs on PATH", `none of them resolve (${missing.join(", ")}); this job could not run a single command.`);
  } else {
    add("warn", "Programs on PATH", `not found, so unavailable: ${missing.join(", ")}`);
  }

  if (!request.model) {
    add("ok", "Model", "the account default");
  } else if (!getSdkStatus().available) {
    add("warn", "Model", `cannot check ${request.model}: the Copilot runtime is not installed here.`);
  } else {
    try {
      await ensureModelAvailable(cwd, request.model);
      add("ok", "Model", request.model);
    } catch (error) {
      add("error", "Model", error.message);
    }
  }

  if (request.reviewName) {
    try {
      ensureGitRepository(cwd);
      // With --pr this reaches GitHub, so the dry run also proves gh is
      // installed, signed in, the number exists, the head is checked out and
      // the base ref is local -- every way the real run can fail, for no
      // Copilot turn.
      const target = resolveReviewTarget(cwd, { base: request.base, scope: request.scope, pr: request.pr });
      add("ok", "Review target", target.label);
    } catch (error) {
      add("error", "Review target", error.message);
    }
  }

  return {
    dryRun: true,
    ready: !checks.some((check) => check.level === "error"),
    workspaceRoot,
    roots: policy.roots.map((entry) => entry.resolved),
    mode,
    checks
  };
}

export async function executeReviewRun(request) {
  await ensureCopilotReady(request.cwd);
  await ensureModelAvailable(request.cwd, request.model);
  ensureGitRepository(request.cwd);

  // The caller resolves the target to title the job, so it is handed down
  // rather than recomputed: with --pr that is a network call, and two of them
  // can disagree. The fallback keeps a worker that reloads a stored request
  // working.
  const target =
    request.target ??
    resolveReviewTarget(request.cwd, {
      base: request.base,
      scope: request.scope,
      pr: request.pr
    });
  const focusText = request.focusText?.trim() ?? "";
  const reviewName = request.reviewName ?? "Review";
  const context = collectReviewContext(request.cwd, target);
  const isAdversarial = reviewName === "Adversarial Review";

  // Both reviews are structured. Upstream left the plain review as a one-line
  // prompt with no schema, which made its output impossible to render or sort
  // by severity the way the review commands promise.
  const prompt = buildReviewPrompt(isAdversarial ? "adversarial-review" : "review", {
    reviewName,
    context,
    focusText
  });

  const permissionMode = request.permissionMode ?? WORKSPACE_EXECUTE;
  const extraPrograms = getConfig(context.repoRoot).extraPrograms ?? [];
  // A review that spans two checkouts -- a sibling library, a shared spec
  // directory -- had no way to say so: --add-dir existed only on a task, so
  // the fence refused every read there with no flag to fix it.
  const additionalDirectories = resolveAdditionalDirectories(request.addDirs, context.repoRoot);
  assertPromptPathsInsideFence(
    focusText,
    createWorkspacePolicy(context.repoRoot, context.repoRoot, additionalDirectories)
  );
  reportMissingPrograms(permissionMode, extraPrograms, request.onProgress);

  const session = await createSession({
    cwd: context.repoRoot,
    workspaceRoot: context.repoRoot,
    additionalDirectories,
    model: request.model,
    reasoningEffort: request.effort,
    // A review never edits -- enforced by the permission handler, not by asking
    // the model nicely -- but it may run the repository's toolchain to check
    // its own conclusions. `--read-only` narrows it back to git and rg.
    permissionMode,
    // Reviews passed none, so `/copilot:setup --allow-programs` never reached
    // the command it was most useful for.
    extraPrograms,
    systemMessage: isAdversarial
      ? "You are Copilot performing an adversarial software review. Return your findings as JSON matching the provided schema."
      : "You are Copilot performing a code review. Return your findings as JSON matching the provided schema."
  });

  const result = await runPrompt(session, prompt, {
    onProgress: request.onProgress,
    // Attaching the repo root lets Copilot open the files around the diff
    // instead of reasoning only about the hunks pasted into the prompt.
    attachments: [{ type: "directory", path: context.repoRoot, displayName: "repository" }],
    agentMode: "plan"
  });
  const rawOutput = result.content ?? "";

  const parsed = parseStructuredOutput(rawOutput);
  // Anything the fence refused -- a read, a command, a program that is not on
  // PATH -- means this review did not see everything it went looking for.
  const denials = result.denials ?? [];
  const degraded = denials.length > 0;
  const payload = {
    review: reviewName,
    target,
    sessionId: result.sessionId,
    context: {
      repoRoot: context.repoRoot,
      branch: context.branch,
      summary: context.summary
    },
    copilot: {
      status: rawOutput ? 0 : 1,
      stdout: rawOutput,
      reasoning: result.reasoning
    },
    result: parsed.parsed,
    rawOutput: parsed.rawOutput,
    parseError: parsed.parseError,
    reasoningSummary: result.reasoning ? [result.reasoning] : [],
    // A review is read-only, so touchedFiles should stay empty; recording
    // both makes a violation visible instead of silently absorbed.
    touchedFiles: result.touchedFiles ?? [],
    denials: result.denials ?? []
  };
  return {
    // 2, not 1: the run produced an answer, but an incomplete one. A caller
    // that only reads the exit code has to be able to tell those apart.
    exitStatus: rawOutput ? (degraded ? 2 : 0) : 1,
    degraded,
    sessionId: result.sessionId,
    copilotSessionId: result.sessionId ?? null,
    threadId: result.sessionId ?? null,
    payload,
    rendered: renderReviewResult(parsed, {
      reviewLabel: reviewName,
      targetLabel: context.target.label,
      denials,
      reasoningSummary: result.reasoning ? [result.reasoning] : []
    }),
    summary: parsed.parsed?.summary ?? parsed.parseError ?? firstMeaningfulLine(rawOutput, `${reviewName} finished.`),
    jobTitle: `Copilot ${reviewName}`,
    jobClass: "review",
    targetLabel: context.target.label,
    metrics: runMetrics(result, prompt)
  };
}

/** What a run cost and did, for the job log. */
function runMetrics(result, prompt) {
  const usage = result.usage ?? {};
  return {
    model: result.model ?? usage.model ?? null,
    // Logged and then thrown away until now, which is why `/copilot:status`
    // could not say what a job had spent.
    apiCalls: usage.apiCalls ?? 0,
    inputTokens: usage.inputTokens ?? null,
    reasoningTokens: usage.reasoningTokens ?? null,
    cost: usage.cost ?? null,
    outputTokens: result.outputTokens ?? null,
    promptBytes: Buffer.byteLength(String(prompt ?? ""), "utf8"),
    toolCalls: Array.isArray(result.toolCalls) ? result.toolCalls.length : 0,
    denials: Array.isArray(result.denials) ? result.denials.length : 0,
    touchedFiles: Array.isArray(result.touchedFiles) ? result.touchedFiles.length : 0,
    escalated: result.escalated === true
  };
}

export async function executeTaskRun(request) {
  const workspaceRoot = resolveWorkspaceRoot(request.cwd);
  await ensureCopilotReady(request.cwd);
  await ensureModelAvailable(request.cwd, request.model);

  const taskMetadata = buildTaskRunMetadata({
    prompt: request.prompt,
    resumeLast: request.resumeLast
  });

  let sessionId = null;
  if (request.resumeLast) {
    const latestThread = await resolveLatestTrackedTaskSession(workspaceRoot, {
      excludeJobId: request.jobId
    });
    if (!latestThread) {
      throw new Error("No previous Copilot task session was found for this repository.");
    }
    sessionId = latestThread.id;
  } else {
    sessionId = buildPersistentTaskSessionId(request.prompt || DEFAULT_CONTINUE_PROMPT);
  }

  if (!request.prompt && !request.resumeLast) {
    throw new Error("Provide a prompt, a prompt file, piped stdin, or use --resume-last.");
  }

  // `--write` decides whether Copilot may edit the tree. Upstream parsed the
  // flag, stored it on the job record, printed it in the report, and never
  // passed it to the session, so every task ran with blanket approval.
  const permissionMode = request.write ? WORKSPACE_WRITE : READ_ONLY;
  // The background worker re-reads the stored request, so the wide-root
  // refusal has to hold here too, not only where the flags were parsed.
  // `--add-dir`: extra roots inside the fence, resolved here (not only where
  // the flags were parsed) because the background worker re-reads the request.
  const additionalDirectories = resolveAdditionalDirectories(request.addDirs, request.cwd);
  assertWriteRootAcceptable(request, workspaceRoot, additionalDirectories);
  const unsafeShell = request.unsafeShell === true;
  const extraPrograms = getConfig(workspaceRoot).extraPrograms ?? [];
  // Both the foreground path and the detached worker come through here, so one
  // check covers both.
  assertPromptPathsInsideFence(
    request.prompt,
    createWorkspacePolicy(workspaceRoot, request.cwd, additionalDirectories)
  );
  reportMissingPrograms(permissionMode, extraPrograms, request.onProgress);
  const sessionOptions = {
    cwd: request.cwd,
    workspaceRoot,
    additionalDirectories,
    model: request.model,
    reasoningEffort: request.effort,
    permissionMode,
    unsafeShell,
    extraPrograms,
    // Reads flagged for the owner pause the job (a sentinel filename). Reads
    // the owner already approved in a prior escalation pass straight through.
    escalateReads: makeSentinelEscalation(),
    approvedReads: new Set(Array.isArray(request.approvedReads) ? request.approvedReads : [])
  };

  const { session, resumed } = request.resumeLast
    ? await resumeSession(sessionId, sessionOptions)
    : {
        session: await createSession({ ...sessionOptions, sessionId }),
        resumed: false
      };
  // Whatever session we actually ended up on: the requested one, or the fresh
  // one resumeSession fell back to.
  const activeSessionId = session?.sessionId ?? sessionId;

  if (unsafeShell) {
    request.onProgress?.({
      message: "Shell tools are unfenced (--unsafe-shell).",
      phase: "starting",
      stderrMessage: "Shell: unfenced (--unsafe-shell)",
      logTitle: null,
      logBody: null
    });
  }

  if (request.resumeLast) {
    request.onProgress?.({
      message: resumed
        ? `Resumed Copilot session ${activeSessionId}.`
        : `Previous session ${sessionId} could not be resumed; started a fresh one (${activeSessionId}).`,
      phase: "starting",
      stderrMessage: resumed
        ? `Resumed session ${activeSessionId}`
        : `Session ${sessionId} was not resumable; started fresh as ${activeSessionId}.`,
      logTitle: null,
      logBody: null
    });
  }

  const promptText = request.prompt || DEFAULT_CONTINUE_PROMPT;
  const result = await runPrompt(session, promptText, {
    onProgress: request.onProgress,
    onPartial: request.onPartial
  });
  const rawOutput = result.content ?? "";
  const failureMessage = "";

  // The policy paused a request for the owner. Hand back an escalated result so
  // the job is stored as awaiting-approval instead of completed, with enough to
  // revive it: the Copilot session id and the fields needed to resume.
  if (result.escalated) {
    const copilotSessionId = result.sessionId ?? activeSessionId;
    return {
      escalated: true,
      exitStatus: 0,
      sessionId: copilotSessionId,
      copilotSessionId,
      threadId: copilotSessionId,
      pendingApproval: result.pendingApproval,
      revive: {
        cwd: request.cwd,
        workspaceRoot,
        model: request.model,
        effort: request.effort,
        write: Boolean(request.write),
        unsafeShell,
        allowWideRoot: request.allowWideRoot === true,
        addDirs: additionalDirectories
      },
      payload: buildTaskPayload(result, { sessionId, rawOutput, unsafeShell }),
      rendered: `${taskMetadata.title} is paused waiting for your approval to ${
        result.pendingApproval?.request ?? "a request"
      }.\nApprove: /copilot:approve\nDeny: /copilot:deny\n`,
      summary: `Paused: ${result.pendingApproval?.request ?? "awaiting approval"}`,
      jobTitle: taskMetadata.title,
      jobClass: "task",
      write: Boolean(request.write),
      unsafeShell,
      metrics: runMetrics(result, promptText)
    };
  }

  const payload = buildTaskPayload(result, { sessionId: activeSessionId, rawOutput, unsafeShell });
  const rendered = renderTaskResult(
    {
      rawOutput,
      failureMessage,
      reasoningSummary: payload.reasoningSummary
    },
    {
      title: taskMetadata.title,
      jobId: request.jobId ?? null,
      write: Boolean(request.write),
      touchedFiles: payload.touchedFiles,
      denials: payload.denials,
      unsafeShell
    }
  );

  const copilotSessionId = result.sessionId ?? activeSessionId;
  const degraded = (payload.denials?.length ?? 0) > 0;
  return {
    exitStatus: rawOutput ? (degraded ? 2 : 0) : 1,
    degraded,
    sessionId: copilotSessionId,
    copilotSessionId,
    threadId: copilotSessionId,
    payload,
    rendered,
    summary: firstMeaningfulLine(rawOutput, firstMeaningfulLine(failureMessage, `${taskMetadata.title} finished.`)),
    jobTitle: taskMetadata.title,
    jobClass: "task",
    write: Boolean(request.write),
    unsafeShell,
    metrics: runMetrics(result, promptText)
  };
}

export function buildTaskRunMetadata({ prompt, resumeLast = false }) {
  if (!resumeLast && String(prompt ?? "").includes(STOP_REVIEW_TASK_MARKER)) {
    return {
      title: "Copilot Stop Gate Review",
      summary: "Stop-gate review of previous Claude turn"
    };
  }

  const title = resumeLast ? "Copilot Resume" : "Copilot Task";
  const fallbackSummary = resumeLast ? DEFAULT_CONTINUE_PROMPT : "Task";
  return {
    title,
    summary: shorten(prompt || fallbackSummary)
  };
}

/**
 * A --write job whose root is the home directory or a drive root would be
 * allowed to edit everything the user owns. Refuse unless explicitly asked.
 */
export function assertWriteRootAcceptable({ write, allowWideRoot }, workspaceRoot, additionalDirectories = []) {
  if (!write || allowWideRoot) return;
  if (isWideRoot(workspaceRoot)) {
    throw new Error(
      `Refused to run a --write task with workspace root ${workspaceRoot}: it is your home directory, an ancestor of it, or a filesystem root, so "inside the workspace" would mean everything. Run from a project directory, or pass --allow-wide-root if you really mean it.`
    );
  }
  // An added root is a root: granting the home directory through --add-dir
  // widens a write job exactly as much as starting it there would.
  for (const directory of additionalDirectories) {
    if (!isWideRoot(directory)) continue;
    throw new Error(
      `Refused to run a --write task with --add-dir ${directory}: it is your home directory, an ancestor of it, or a filesystem root. Name a project directory, or pass --allow-wide-root if you really mean it.`
    );
  }
}

/**
 * Revive a job that was paused on the owner's approval. Resumes the Copilot
 * session (full history), allows the approved path, and re-instructs the model
 * to redo the action. A pending permission cannot be frozen and continued, so
 * the model re-attempts instead.
 */
export async function executeApprovalResume(request) {
  const workspaceRoot = request.workspaceRoot ?? resolveWorkspaceRoot(request.cwd);
  await ensureCopilotReady(request.cwd);
  // The revive context carries the original --write; the wide-root refusal
  // holds on the way back in as well.
  const resolvedAddDirs = resolveAdditionalDirectories(request.addDirs, request.cwd);
  assertWriteRootAcceptable(
    { write: request.write, allowWideRoot: request.allowWideRoot },
    workspaceRoot,
    resolvedAddDirs
  );

  const permissionMode = request.write ? WORKSPACE_WRITE : READ_ONLY;
  const approvedFile = request.pendingApproval?.file ?? null;
  const { session, resumed } = await resumeSession(request.copilotSessionId, {
    cwd: request.cwd,
    workspaceRoot,
    additionalDirectories: resolvedAddDirs,
    model: request.model,
    reasoningEffort: request.effort,
    permissionMode,
    unsafeShell: request.unsafeShell === true,
    extraPrograms: getConfig(workspaceRoot).extraPrograms ?? [],
    escalateReads: makeSentinelEscalation(),
    approvedReads: new Set(approvedFile ? [approvedFile] : []),
    // An approval-resume must reuse the frozen context; a fresh session would
    // silently restart the whole task. If the state is gone, the job expired.
    allowFreshFallback: false
  });

  if (!resumed || !session) {
    return {
      expired: true,
      exitStatus: 1,
      sessionId: request.copilotSessionId,
      payload: { status: 1, sessionId: request.copilotSessionId, rawOutput: "", touchedFiles: [], denials: [] },
      rendered: `This job's Copilot session (${request.copilotSessionId}) is no longer available, so it cannot be resumed. Re-run the original task.\n`,
      summary: "Session expired before approval",
      jobClass: "task"
    };
  }

  const target = request.pendingApproval?.request ?? (approvedFile ? `read ${approvedFile}` : "the paused action");
  const reinstruct = `The owner has approved the request that was paused (${target}). Please carry it out now and continue the task.`;
  const result = await runPrompt(session, reinstruct, {
    onProgress: request.onProgress,
    onPartial: request.onPartial
  });
  const rawOutput = result.content ?? "";

  // A second, different escalation can happen; re-suspend on the same path.
  if (result.escalated) {
    const copilotSessionId = result.sessionId ?? request.copilotSessionId;
    return {
      escalated: true,
      exitStatus: 0,
      sessionId: copilotSessionId,
      copilotSessionId,
      threadId: copilotSessionId,
      pendingApproval: result.pendingApproval,
      revive: {
        cwd: request.cwd,
        workspaceRoot,
        model: request.model,
        effort: request.effort,
        write: Boolean(request.write),
        unsafeShell: request.unsafeShell === true,
        allowWideRoot: request.allowWideRoot === true,
        addDirs: resolvedAddDirs
      },
      payload: buildTaskPayload(result, { sessionId: copilotSessionId, rawOutput, unsafeShell: request.unsafeShell === true }),
      rendered: `Paused again waiting for your approval to ${result.pendingApproval?.request ?? "a request"}.\nApprove: /copilot:approve\nDeny: /copilot:deny\n`,
      summary: `Paused: ${result.pendingApproval?.request ?? "awaiting approval"}`,
      jobClass: "task",
      metrics: runMetrics(result, reinstruct)
    };
  }

  const payload = buildTaskPayload(result, {
    sessionId: request.copilotSessionId,
    rawOutput,
    unsafeShell: request.unsafeShell === true
  });
  const rendered = renderTaskResult(
    { rawOutput, failureMessage: "", reasoningSummary: payload.reasoningSummary },
    {
      title: request.title ?? "Copilot Task",
      jobId: request.jobId ?? null,
      write: Boolean(request.write),
      touchedFiles: payload.touchedFiles,
      denials: payload.denials,
      unsafeShell: request.unsafeShell === true
    }
  );
  const finalSessionId = result.sessionId ?? request.copilotSessionId;
  const degraded = (payload.denials?.length ?? 0) > 0;
  return {
    exitStatus: rawOutput ? (degraded ? 2 : 0) : 1,
    degraded,
    sessionId: finalSessionId,
    copilotSessionId: finalSessionId,
    threadId: finalSessionId,
    payload,
    rendered,
    summary: firstMeaningfulLine(rawOutput, "Resumed after approval."),
    jobClass: "task",
    write: Boolean(request.write),
    metrics: runMetrics(result, reinstruct)
  };
}
