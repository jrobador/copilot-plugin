...
/**
 * Programs the Copilot runtime ships inside its own platform package, and
 * where each one lives relative to that package's root.
 *
 * ripgrep is the one that matters: `rg` is on the allowlist of every job, but
 * it is absent from most machines, and a review that cannot search is a review
 * with a hole in it. The runtime we already depend on carries a copy, so the
 * answer is to use it rather than to tell the model its search tool is gone.
 */
const BUNDLED_BINARIES = Object.freeze({
  rg: (platform, arch, exe) => ["ripgrep", "bin", `${platform}-${arch}`, `rg${exe}`]
});

/**
 * Locate a program inside the installed Copilot runtime, or null.
 *
 * Best effort by construction: the runtime is an optional dependency, the
 * platform package may not be installed, and its internal layout is not a
 * published contract. Every failure returns null and the caller falls back to
 * PATH, so a layout change downgrades to today's behaviour instead of breaking.
 *
 * @param {string} name
 * @param {{platform?: string, arch?: string}} [options]
 * @returns {string|null}
 */
export function bundledBinary(name, options = {}) {
  const segments = BUNDLED_BINARIES[name];
  if (!segments) return null;
  const platform = options.platform ?? process.platform;
  const arch = options.arch ?? process.arch;
  try {
    const require = createRequire(import.meta.url);
    // The platform package's main entry is the CLI executable itself, so its
    // directory is the package root. `package.json` is not exported, so it
    // cannot be resolved by name.
    const packageRoot = path.dirname(require.resolve(`@github/copilot-${platform}-${arch}`));
    const candidate = path.join(packageRoot, ...segments(platform, arch, platform === "win32" ? ".exe" : ""));
...
/**
 * Where a program this job is allowed to run actually lives: PATH first, then
 * the copy bundled with the Copilot runtime.
 *
 * Every caller that asks "can this job run X?" must go through this and not
 * through resolveBinary, or the pre-flight warning and the launch disagree:
 * one reports a program as missing that the other then runs.
 */
export function resolveProgram(command, env = process.env) {
  return resolveBinary(command, env) ?? bundledBinary(command);
}
