...
/** @type {Array<{name: string, description: string, inputSchema: object, toArgv: (args: object) => string[]}>} */
export const TOOLS = [
  {
    name: "copilot_review",
    description:
      "Read-only Copilot review of your uncommitted changes or your branch. Returns findings ordered by severity. Blocks until the review finishes.",
  ...
  },
  {
    ...
    description:
      "Read-only Copilot review that challenges the design, not just the code: auth, data loss, rollback safety, races, version skew. Takes optional free-text focus.",
    inputSchema: {
      type: "object",
  ...
  },
  {
    name: "copilot_rescue",
    description:
      "Hand a task to Copilot: investigate, diagnose, or fix. Read-only unless `write` is true. Use `background: true` for long tasks and poll copilot_status / copilot_result.",
    inputSchema: {
      type: "object",
      properties: {
        ...PATH_PROP,
        prompt: { type: "string", description: "What Copilot should do." },
        write: { type: "boolean", description: "Allow Copilot to edit files. Default false (reports a diff instead)." },
        ...
        unsafe_shell: { type: "boolean", description: "Restore the runtime's own shell tools. Rarely needed." },
        allow_wide_root: { type: "boolean", description: "Permit a --write task rooted at the home directory or a drive root." },
        ...MODEL_PROPS
      }
    },
    toArgv: (a) => [
      ...
      ...flag("--unsafe-shell", a.unsafe_shell),
      ...flag("--allow-wide-root", a.allow_wide_root),
      ...opt("--model", a.model),
      ...opt("--effort", a.effort),
