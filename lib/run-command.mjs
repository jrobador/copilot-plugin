 ...
 *
 * That ceiling matters most in WORKSPACE_EXECUTE, the mode a review runs in:
 * "execute everything, mutate nothing" is a promise about what Copilot may
 * ask for, not about what the repository's own test suite does once started.
 * A suite writes caches, deletes files and opens sockets, and none of that
 * reaches the permission handler, which only ever sees the tool call.
 *
...
import { resolveProgram, terminateProcessTree } from "./process.mjs";

/**
 * What to reach for instead, when a program this job asked for is not
 * available to it. A denial that only says no costs a turn while the model
 * re-derives what it is allowed to do -- and, in the runs that motivated this,
 * several turns arguing with the wall instead of reviewing the code.
 *
 * Only programs a job plausibly asks for and that have a real in-fence
 * equivalent belong here. `gh` is the one that is never coming: it is outbound
 * network with the user's token, which is the same surface a non-write job
 * closes by refusing URL fetches.
 */
const PROGRAM_SUBSTITUTES = Object.freeze({
  rg: "search with `git grep -n <pattern> -- <path>` instead",
  gh: "GitHub is not reachable from inside a job; a pull request's diff and description are already in the prompt when the review was started with --pr"
});

/** ` Use X instead.`-style tail for a denial, or "" when there is no advice. */
function substituteHint(name) {
  const hint = PROGRAM_SUBSTITUTES[name];
  return hint ? ` ${hint}.` : "";
}
...
/**
 * git operations a write job may not run. A --write job is scoped to the
 * workspace, so publishing to a remote or reading the credential helper is out
 * of scope by definition; `reset --hard` and `clean -f` destroy uncommitted
 * work the owner never handed over, and `stash`, `restore`, `checkout` and
 * `switch` hide or overwrite it (see `worktreeGitDenial`). Everything else a
 * job needs to do its work -- add, commit, branch, merge, rebase -- stays
 * allowed, and so does creating a branch with `checkout -b` or `switch -c`.
 *
 * Without this, the write path skipped every git check and the `.git/**`
 * protection did not apply: those subcommands take refs and config keys, not
 * paths, so `looksLikePath` never sees them.
 */
export const WRITE_DENIED_GIT_SUBCOMMANDS = Object.freeze({
  push: "publishes outside the workspace this job is scoped to",
  credential: "prints stored credentials",
  // `git config alias.x '!sh -c ...'` defines a subcommand that runs through a
  // shell, and `git x` then executes it: the whole fence -- the program
  // allowlist, the path checks, the no-shell rule -- goes around it in two
  // allowed commands. Denying --global/--system is not enough, because the
  // repository's own config is inside the workspace.
  config: "can define a shell alias that runs outside the fence"
});

/** `-b <name> [start-point]`: a new name, and nothing that looks like an option or a path list. */
function createsBranchOnly(rest, flags) {
  if (!flags.includes(rest[0]) || !rest[1] || rest.length > 3) return false;
  return rest.slice(1).every((arg) => !arg.startsWith("-"));
}

/**
 * Why a write job may not run this form of stash, restore, checkout or switch,
 * or null when the form is one of the few allowed.
 *
 * A job often shares its working tree with the owner's uncommitted work and
 * with other jobs. These four subcommands hide that work, overwrite it with
 * committed content, or move HEAD under it. One observed case: a job stashed
 * to compare against the base; had it stopped before `stash pop`, another
 * job's finished changes would have left the tree without a trace.
 */
export function worktreeGitDenial(subcommand, rest) {
  switch (subcommand) {
    case "stash":
      return ["list", "show"].includes(rest[0])
        ? null
        : "hides uncommitted work, and a job that stops before `stash pop` leaves it hidden. Only `git stash list` and `git stash show` are available";
    case "restore": {
      const staged = rest.includes("--staged") || rest.includes("-S");
      const worktree = rest.includes("--worktree") || rest.includes("-W");
      return staged && !worktree
        ? null
        : "overwrites files in the working tree with committed content. Only `git restore --staged <path>` is available";
    }
    case "checkout":
      return createsBranchOnly(rest, ["-b"])
        ? null
        : "can overwrite files, or move HEAD under work the job was not given. Only `git checkout -b <new-branch> [start-point]` is available";
    case "switch":
      return createsBranchOnly(rest, ["-c", "--create"])
        ? null
        : "moves HEAD under work the job was not given. Only `git switch -c <new-branch> [start-point]` is available";
    default:
      return null;
  }
}

/** Per-subcommand flags that discard work irreversibly. */
export const WRITE_DENIED_GIT_FLAGS = Object.freeze({
  reset: ["--hard"],
  clean: ["-f", "--force", "-d", "-x"]
});
...
export const DENIED_FLAGS = Object.freeze({
  ...
  // --global/--system reach the user's own config, which no job is scoped to.
  git: ["--git-dir", "--work-tree", "--exec-path", "--namespace", "--config-env", "--global", "--system"],
...
export function allowedPrograms(mode, config = {}) {
  ...
  // Only a strictly read-only job is held to git and rg. A review may run the
  // repository's toolchain -- that is the whole point of workspace-execute --
  // while the permission handler keeps refusing every write it asks for.
  const base = effectiveMode === READ_ONLY ? READ_ONLY_PROGRAMS : WRITE_PROGRAMS;
  // extraPrograms is the workspace owner's own grant, so it applies in every
  // mode. Gating it on --write made `/copilot:setup --allow-programs` dead for
  // exactly the jobs that most need an extra tool.
  const extra = (Array.isArray(config.extraPrograms) ? config.extraPrograms : [])
...
/**
 * Programs this job is allowed to run that this machine cannot actually run.
 * Never fatal on its own -- python3, dotnet, go and cargo are absent from most
 * machines -- but a job that silently cannot run its own test suite is worth
 * one line of warning before the turn rather than a denial in the middle of it.
 */
export function missingPrograms(mode, config = {}, env = process.env) {
  return allowedPrograms(mode, config).filter((program) => !resolveProgram(program, env));
}

/** What a job that may not mutate the repository calls itself in a git denial. */
function nonWriteLabel(mode) {
  return normalizeMode(mode) === READ_ONLY ? "read-only job" : "non-mutating job";
}
...
function planGit(args, effectiveMode) {
  ...
  if (effectiveMode === WORKSPACE_WRITE) {
    if (subcommand && Object.hasOwn(WRITE_DENIED_GIT_SUBCOMMANDS, subcommand)) {
      return { error: `git ${subcommand} is not available to a write job: it ${WRITE_DENIED_GIT_SUBCOMMANDS[subcommand]}.` };
    }
    const destructive = subcommand && Object.hasOwn(WRITE_DENIED_GIT_FLAGS, subcommand)
      ? findDeniedFlag(rest, WRITE_DENIED_GIT_FLAGS[subcommand])
      : null;
    if (destructive) {
      return { error: `git ${subcommand} ${destructive.arg} discards work that was never handed to the job; it is not available even with --write.` };
    }
    const worktreeReason = worktreeGitDenial(subcommand, rest);
    if (worktreeReason) {
      return { error: `git ${subcommand} is not available to a write job in this form: it ${worktreeReason}.` };
    }
  }

  if (effectiveMode !== WORKSPACE_WRITE) {
    if (!subcommand) {
      return { error: `git needs a subcommand; a ${nonWriteLabel(effectiveMode)} may use: ${READ_ONLY_GIT_SUBCOMMANDS.join(", ")}.` };
    }
    if (!READ_ONLY_GIT_SUBCOMMANDS.includes(subcommand)) {
      return { error: `git ${subcommand} is not available to a ${nonWriteLabel(effectiveMode)}; allowed subcommands: ${READ_ONLY_GIT_SUBCOMMANDS.join(", ")}.` };
    }
    if (subcommand === "branch") {
      const positional = rest.find((arg) => !arg.startsWith("-"));
      if (positional !== undefined) {
        return { error: `git branch with a positional argument (\`${positional}\`) creates or targets a branch; a ${nonWriteLabel(effectiveMode)} may only list (use --list, -a, -r, --contains=<ref>).` };
      }
      const mutating = findDeniedFlag(rest, GIT_BRANCH_MUTATING_FLAGS);
      if (mutating) {
        return { error: `git branch ${mutating.arg} modifies branches; a ${nonWriteLabel(effectiveMode)} may only list.` };
      }
    }
    if (subcommand === "remote") {
      ...
      if (!listing) {
        return { error: `git remote in a ${nonWriteLabel(effectiveMode)} may only list: \`remote\`, \`remote -v\`, \`remote show <name>\`, \`remote get-url <name>\`.` };
      }
    }
  }
...
export function planCommand(request, mode, policy, config = {}) {
  ...
  if (!allowed.includes(name)) {
    // The list this job actually has, in every mode. Naming the read-only pair
    // to an execute job -- which has the whole toolchain -- sent the model off
    // to re-derive its own permissions from a denial that was simply wrong.
    const gitNote = effectiveMode === WORKSPACE_WRITE ? "" : "; git is limited to its read subcommands";
    return refuse(
      `\`${program}\` is not on the ${effectiveMode} allowlist (allowed: ${allowed.join(", ")}${gitNote}).${substituteHint(name)}`
    );
  }
...
export function resolveLaunch(plan, options = {}) {
  ...
  const resolved = resolveProgram(plan.program, env);
  if (!resolved) {
    // A denial, not a failure. The job was promised this program by its mode,
    // and not having it is a hole in what the job could check -- exactly what
    // the degraded marker exists to catch. Reported as a plain failure, it
    // reached the caller as an *allowed* command that happened to say nothing.
    return {
      ok: false,
      denial: true,
      reason: `${plan.program} is allowed for this job but was not found on PATH; it cannot be run here.${substituteHint(plan.program)}`
    };
  }
...
/** The roots a path argument may live in: the workspace, plus any `--add-dir`. */
function describeFence(policy) {
  const roots = (policy.roots ?? []).map((entry) => entry.resolved);
  return roots.length > 1 ? roots.join(" or ") : policy.root;
}

export function buildRunCommandDescription(mode, policy, config = {}) {
  const effectiveMode = normalizeMode(mode);
  const programs = allowedPrograms(effectiveMode, config);
  const gitNote = ` Allowed git subcommands: ${READ_ONLY_GIT_SUBCOMMANDS.join(", ")} (branch and remote in listing form only; use --flag=value forms).`;
  const readOnlyNote =
    effectiveMode === WORKSPACE_WRITE
      ? ""
      : effectiveMode === WORKSPACE_EXECUTE
        ? ` This job may run the programs above to check its own conclusions, but may not change anything: writes, URL fetches and mutating git subcommands are refused.${gitNote}`
  ...
  return (
    ...
    `The working directory is ${policy.cwd} and every path argument must stay inside ${describeFence(policy)}; .git/, .github/workflows/, .husky/ and .vscode/tasks.json are off limits. ` +
...
/**
 * The start and the end of a command's output. Keeping only the start lost
 * what a test run prints last, its pass/fail summary, behind a screen of
 * config warnings: the log said `exit 1` and could not say why.
 */
export function previewOf(output, headLines = 5, tailLines = 15, maxChars = 2000) {
  const lines = String(output ?? "").trimEnd().split("\n");
  const kept =
    lines.length <= headLines + tailLines
      ? lines
      : [...lines.slice(0, headLines), `… ${lines.length - headLines - tailLines} lines omitted …`, ...lines.slice(-tailLines)];
