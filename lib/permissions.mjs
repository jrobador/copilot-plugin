...
import { canonicalizePath, createWorkspacePolicy, isInsideWorkspace } from "./paths.mjs";

/** Review and diagnosis: Copilot may look, never touch. */
export const READ_ONLY = "read-only";
/**
 * Reviews: Copilot may run the repository's own toolchain, but may not change
 * anything. The read/write axis was the wrong one for a review -- confirming a
 * hypothesis means running the tests, and the only mode that allowed that also
 * handed out an editor. This is the crossed axis: execute everything, mutate
 * nothing.
 */
export const WORKSPACE_EXECUTE = "workspace-execute";
/** Rescue tasks: Copilot may edit the workspace it was pointed at. */
export const WORKSPACE_WRITE = "workspace-write";

export const PERMISSION_MODES = new Set([READ_ONLY, WORKSPACE_EXECUTE, WORKSPACE_WRITE]);
...
/**
 * How a job that may not write describes itself in a denial. Both non-write
 * modes refuse a write, but for different reasons: telling a model it is
 * "read-only" when it may in fact run the test suite costs a turn while it
 * re-derives what it is allowed to do.
 */
function modeLimit(mode) {
  return mode === WORKSPACE_EXECUTE
    ? "this job may run commands but may not modify anything"
...
function resolvePolicy(policy) {
  ...
  return createWorkspacePolicy(root, policy?.cwd ?? root, policy?.additionalDirectories ?? []);
}
...
function outsideReason(check, policy) {
  ...
  const extras = (policy.roots ?? []).slice(1).map((entry) => entry.resolved);
  const fence = extras.length > 0 ? `${policy.root} (or ${extras.join(", ")})` : policy.root;
  return `it resolves to ${check.resolved}, outside the workspace ${fence}`;
}
...
/**
 * Basenames the Copilot runtime gives the files it spools its own large tool
 * output into. Lifted from the pattern the runtime itself carries to
 * recognize them; keep the two in step if the runtime's changes.
 */
const TOOL_OUTPUT_SPOOL = /^(?:\d+-copilot-tool-output-|copilot-tool-output(?:-original)?-|original-output-\d+-)/i;

/**
 * Is this the runtime reading back output it just wrote itself?
 *
 * When a tool produces more than the runtime wants to hold in memory, it
 * writes the rest to a file in the OS temp directory and then reads it back.
 * That read is outside the workspace, so the fence refused it -- and the job
 * was marked degraded for failing to see output that was generated inside it
 * and never left the machine. Refusing it protected nothing and cost the
 * review the evidence it had just produced.
 *
 * Narrow on purpose: the file must sit directly in the temp directory and
 * carry the runtime's own name. The ceiling is that another process on this
 * machine could plant a file there under that name; anything able to do that
 * already runs as the user, and the payoff is a read the job was entitled to.
 */
function isRuntimeToolOutput(resolved, tmpdir = os.tmpdir()) {
  if (typeof resolved !== "string" || resolved === "") return false;
  const directory = path.dirname(resolved);
  const key = (text) => (IS_WINDOWS ? text.toLowerCase() : text);
  let tempCanonical;
  try {
    tempCanonical = canonicalizePath(path.resolve(tmpdir));
  } catch {
    return false;
  }
  if (key(directory) !== key(tempCanonical)) return false;
  return TOOL_OUTPUT_SPOOL.test(path.basename(resolved));
}
 ...
 * @param {object} [policy] `{ workspaceRoot, cwd, additionalDirectories }` or a createWorkspacePolicy
...
export function decidePermission(request, mode = READ_ONLY, policy = {}) {
  ...
  switch (kind) {
    case "read": {
      ...
      if (!check.inside) {
        // One exception, and it is the runtime reading back its own spooled
        // tool output. See isRuntimeToolOutput.
        if (isRuntimeToolOutput(check.resolved)) {
          return allow("Reading back the runtime's own spooled tool output.");
        }
        return deny(
      ...
      // Keyed by the path relative to the root that matched, so with --add-dir
      // an approved `src/x.js` also covers `src/x.js` in another added root.
      // ponytail: both roots were granted by the owner in the same command;
      // key by `${check.root}|${check.relative}` if that stops being true.
    ...
    case "write": {
      ...
      return effectiveMode === WORKSPACE_WRITE
        ...
        : deny(
            `Refused to write ${request.fileName ?? "a file"}: ${modeLimit(
              effectiveMode
            )}. Report the change as a suggested diff instead of applying it.`
          );
    }
    ...
    case "shell": {
      ...
      if (request.hasWriteFileRedirection) {
        return deny(
          `Refused \`${request.fullCommandText}\`: it redirects output to a file and ${modeLimit(effectiveMode)}.`
        );
      }
      if (mutating.length > 0) {
        const names = mutating.map((command) => command.identifier ?? "?").join(", ");
        return deny(
          `Refused \`${request.fullCommandText}\`: ${names} can modify state and ${modeLimit(effectiveMode)}.`
        );
      }
      ...
      return effectiveMode === WORKSPACE_WRITE
        ...
        : deny(
            effectiveMode === WORKSPACE_EXECUTE
              ? `Refused to fetch ${request.url ?? "a URL"}: this job may run the repository's own commands, but the model may not fetch URLs.`
              : `Refused to fetch ${request.url ?? "a URL"}: this job is read-only and offline.`
          );
      ...
      return effectiveMode === WORKSPACE_WRITE
        ...
        : deny(
            `Refused MCP tool ${request.serverName ?? "?"}/${
              request.toolName ?? "?"
            }: it is not read-only and ${modeLimit(effectiveMode)}.`
          );
...
export function createPermissionHandler(mode, onDecision, scope = {}) {
  ...
  const workspace = createWorkspacePolicy(root, scope.cwd ?? root, scope.additionalDirectories ?? []);
