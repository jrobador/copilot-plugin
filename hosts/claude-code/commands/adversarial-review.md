...
- Run:
```bash
node "${CLAUDE_PLUGIN_ROOT}/bin/copilot-plugin.mjs" adversarial-review "$ARGUMENTS"
```
...
```typescript
Bash({
  command: `node "${CLAUDE_PLUGIN_ROOT}/bin/copilot-plugin.mjs" adversarial-review "$ARGUMENTS"`,
