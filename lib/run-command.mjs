...
export function missingPrograms(mode, config = {}, env = process.env) {
  return allowedPrograms(mode, config).filter((program) => !resolveBinary(program, env));
}
...
export function resolveLaunch(plan, options = {}) {
  ...
  if (!resolved) {
    ...
    return {
      ok: false,
      denial: true,
      reason: `${plan.program} is allowed for this job but was not found on PATH; it cannot be run here.`
    };
  }
