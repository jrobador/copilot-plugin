...
- **A third permission mode, `workspace-execute`, and it is now the default for both reviews.** The read/write axis was the wrong one for a review: confirming a hypothesis means running the tests, and the only mode that allowed that also handed out an editor. The new mode is the crossed axis — run the repository's whole toolchain (`npm`, `node`, `pytest`, `cargo`, `make`, …), change nothing. Writes, URL fetches and mutating git subcommands stay refused by the permission handler, so `--write` remains the only way to edit anything. `--read-only` narrows a review back to `git` and `rg`. The review prompts now say when running a command is warranted, because a mode the prompt never mentions is a mode the model never uses. The ceiling is documented rather than papered over: `run_command` fences which program starts, not what it does once started, so a test suite still writes its own caches and opens its own sockets.
- **Degraded runs.** A run that was refused anything — a read outside the fence, a command the mode does not allow, a program missing from PATH — is now marked in four places: job status `completed-degraded`, exit status **2** (distinct from 1, which means no output at all), a `DEGRADED RUN` banner above the conclusion, and, for a review, `Verdict: unreliable` in place of the model's word. Three models reported "no findings" on this repository after being denied most of the files they asked for, and nothing in the output said so. A silent false negative is worse than a loud error: it ends the search.
- **`--dry-run` on `task`, `review` and `adversarial-review`.** Validates the workspace root, every `--add-dir`, the paths the prompt names, the model, and which allowed programs actually resolve on PATH, then prints the list and exits. No session, no turn, no cost.
- **Real `--help` on every subcommand**, intercepted before any parsing, state write or SDK load. `task --help` used to fall through to the prompt: it opened a session and paid for a turn having the model answer the literal text `--help`.
- **A pre-flight path check.** A prompt that names a file outside the job's fence now fails before the turn starts, naming the offender and the `--add-dir` that fixes it, instead of letting the model find the wall by walking into it. Only unambiguous, existing paths are reported: a false positive would block a legitimate prompt.
- **Live token, cost and output reporting.** The runtime's `assistant.usage` and `tool.execution_progress` events were being dropped on the floor; they now feed the progress log and `job.metrics`, so `/copilot:status` can say what a job has spent. A background job also writes its output-so-far to its job record, so `/copilot:status <id>` shows something before the turn ends. This deliberately does **not** read the Copilot CLI's private `~/.copilot/session-state/*/events.jsonl`: the SDK marks exactly those three signals `ephemeral`, so they are not in that file anyway.
- **A worker watchdog and a stale-job reaper.** A detached worker re-reads its own job record every 30 seconds and stops itself if the job was cancelled elsewhere or if it passes a 45-minute lifetime cap; `/copilot:status` and every background enqueue close out jobs whose worker is already gone. Previously only a clean `SessionEnd` hook cancelled anything, so a caller killed abruptly left workers billing in silence. (A worker cannot watch the process that spawned it: that one exits seconds later by design.)

- **`--add-dir <path>` on a rescue**, repeatable, matching the flag the raw Copilot CLI has and the wrapper did not expose. Each added directory becomes a root of the same fence: readable in any job, writable in a `--write` one, and valid as a `run_command` path argument, while `.git/`, CI workflows and hooks stay protected inside it. The directories also go to the runtime as `additionalDirectories`, so the model sees them. A path that is not an existing directory is refused rather than skipped, and `--add-dir` naming your home directory or a drive root is refused in a `--write` job unless `--allow-wide-root` is set. It is not exposed over MCP, where the arguments come from a model rather than from you.

The rest of this release is four defects found by two independent adversarial Copilot reviews. Three are permission-fence gaps: the fence was there, but the paths that actually run went around it.
...
- **An unknown flag is now an error, everywhere.** It used to become part of the prompt: `task --backgruond "fix x"` sent the typo to the model and paid for it, and a caller that forgot to strip `--wait` did the same. Free text that starts with a dash goes after `--`. `--read-only` and `--wait` are now registered flags on `task` (the latter a no-op: blocking is already the default).
- **`/copilot:setup --allow-programs` applies in every mode.** The extra programs were silently dropped outside `--write`, which made the setting dead for reviews — the jobs most likely to need one more tool. Anyone who has already allowed programs will see them apply to reviews after upgrading.
- **A program that is allowed but missing from PATH is a denial, not a silent failure.** It used to be reported to the job as an allowed command that happened to produce nothing, which kept it out of the denial list and out of the degraded marker — the exact case where a model reported `rg` was unavailable and the run still looked clean.
- `completed-degraded` joins the terminal job statuses. A record written by this version and read by an older one is neither terminal nor in-play there, so that older version will refuse to show it and will not prune it.
